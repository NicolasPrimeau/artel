...
**Secrets are redacted on the way in.** Sessions are full of pasted keys, and a capture is read by the archivist's model, so the server strips well-known key formats (cloud, payment, model-provider and source-host keys, private key blocks) and secret-looking `KEY=value` assignments before a capture is stored. Anything a pattern cannot recognize still goes through, so treat a pasted secret as exposed and rotate it.
